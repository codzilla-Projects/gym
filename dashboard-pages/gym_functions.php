...
function gym_register_custom_menu_pages() {
  ...
  'Add Coach Abscence',
  'manage_options',
  'insert_coach_abscence',
  'ha_insert_abscence_callback'
  );
  add_menu_page( 
    'Coaches', 
    'Coaches', 
...
require_once ( SH_ROOT . 'dashboard-pages/coach_abscence.php');
require_once ( SH_ROOT . 'dashboard-pages/insert_coach_abscence.php');
...
/*========================================*/
function sh_validate_edit_profile_coach($user_id,$post_data,$profile_img){
  ...
  return sh_edit_profile_coach($user_id,$firstname, $lastname , $phone, $address ,$profile_img);


}


function sh_edit_profile_coach($user_id,$firstname,$lastname,$phone,$address,$profile_img){
...
function tf_after_login_redirection_by_user_roles( $redirect_to, $request, $user ) {
  ...
  if ( isset( $user->roles ) && is_array( $user->roles ) ) :
    ...
    if ( in_array( 'trainee', $user->roles ) || in_array( 'caoch', $user->roles ) ) :

      $pages = get_pages(array(
        'meta_key' => '_wp_page_template',
        'meta_value' => 'page-templates/dashboard-index.php'
      ));
      $pullit_trainee_coaches_db = get_page_link($pages[0]->ID);
    ...
    elseif ( in_array( 'moderator', $user->roles ) || in_array( 'gym-admin', $user->roles ) ) :
      ...
      $pullit_mod_admin_db = get_page_link($pages[0]->ID);
...
function user_entered_gym($gym_id = false, $user_id = false) {
  if( empty($gym_id) || empty($user_id) ) return false;
  update_user_meta($user_id, 'user_in', time());
...
function user_left_gym($gym_id = false, $user_id = false) {
  ...
  delete_user_meta($user_id, 'user_in');
...
function record_staff_attendance($staff_id = false) {
  ...
  $user_in = get_user_meta($staff_id, 'sl_user_in', true);
  if (!$user_in) {
    $event = 1;
    update_user_meta($staff_id, 'sl_user_in', '1');
  }
  else {
    $event = 2;
    delete_user_meta($staff_id, 'sl_user_in');
  }
  $event = !$user_in ? 1 : 2;
...
function get_in_gym_users($gym_id = false) {
  ...
  $sql = "SELECT `user_id`, `meta_value` from `{$wpdb->prefix}usermeta` WHERE `user_id` IN ({$users_ids}) AND `meta_key` = 'user_in'";
